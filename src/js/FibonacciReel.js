$.fn.FibonacciReel = function() {
    ...
    */
    ...
    this.getBorderWidthOffset = function(elem) {
        return parseFloat(elem.css('border-left-width').replace('px',''))+parseFloat(elem.css('border-right-width').replace('px',''));
    };
    
    this.getBorderHeightOffset = function(elem) {
        return parseFloat(elem.css('border-top-width').replace('px',''))+parseFloat(elem.css('border-bottom-width').replace('px',''));
    };

    this.append = function() {
        if(!me.at) me.at = 0;
        if(!me.rowAt) me.rowAt = 1;
        var row_tally=0;
        ...
        do {
            ...
            var thisrow = [];
            w = w-options.gutter;
            console.log("Row: " + (me.rowAt));
            for(var j=0; j<me.rowCnt && me.at<options.data.length; j++, me.at++) {
                var item = options.data[me.at];
                // append to container
                $(options.container).append("<li id=\"" + item.id + "\" data-row=\"" + me.rowAt + "\">" + item.html + "</li>");
                var DOMitem = $('#'+item.id);
                DOMitem.addClass(options.itemClass);
                DOMitem.hide();
                var adj_w = w-(me.getBorderWidthOffset(DOMitem)+(options.contentPadding*2));
                var adj_h = options.height-(me.getBorderHeightOffset(DOMitem)+(options.contentPadding*2));
                DOMitem.css({
                    width: adj_w,
                    height: adj_h,
                    padding: options.contentPadding, float: "left", display: "inline-block", 
                    margin: "0 " + options.gutter + "px " + options.gutter + "px 0"
                });
                DOMitem.fadeIn(250);
                if(item.img) {
                    // track width
                    DOMitem.css({
                        /*height: options.height-(options.contentPadding*2),
                        width: w-(options.contentPadding*2),
                        margin: "0 " + options.gutter + "px " + options.gutter + "px 0",
                ...
                DOMitem.bind('mouseover click', function() {
                    ...
                    //console.log(me.getBorderHeightOffset(tItem));
                });
                me.lastItem = item;
                // push to row last
    ...
    this.constructFromDS = function() {
        $.ajax({
            url: options.api_datasource,
            type: "post",
            success: function(data, status, jqXHR) {
                // console.log(data);
                var src_data;
                if(jQuery.isFunction(options.api_format_callback)) {
                    src_data = options.api_format_callback(data);
                }
                else if(data[options.api_index]) {
